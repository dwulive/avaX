...
(function(){
function avaMain() {
function avaInit() {  console.error("I am in avaInit");  paDebug("test paDebug");qx.Class.define("ava.Version", {
    type: "static",
    statics: {
        PAversion: "0.0.3",
        PAbuild: "10:09:13"
...
var incomingAttackInfo = (function () {
    function incomingAttackInfo() {
    }
                    ...
                    for (var i = 0; i < this._incomingAttacks.length; i++) {
                        try  {
                            ...
                            if ((continent == "-1" || cont == continent) && (!filterOwn || item.tpn == this.pName)) {
                                ...
                                rowData.push([
                                    (item.m ? "webfrontend/world/icon_wm_city_moongate.png" : ""),
                                    (isInternal ? "Internal " : ""),
                                    item.tpn,
                                    item.tcn,
                                    cont,
                                    ava.CoordUtils.convertIdToCoodrinates(item.tc),
                                    formatIncomingDate(this.serverTime.getStepTime(item.es)),
                                    item.pn,
                                    item.an,
                                    item.cn,
                                    ava.CoordUtils.convertIdToCoodrinates(item.c),
                                    formatIncomingDate(this.serverTime.getStepTime(item.ds)),
                                    incomingStr[4],
                                    incomingStr[3],
                                    incomingStr[2],
                                    incomingStr[1],
                                    ...
                                    IncomingShip,
                                    item.a.toString(),
                                    (besieged ? "~" : "") + FormatTime(travelDurationMs),
                                    item.td,
                                    item.td,
                                    "0%",
                                    "0%",
            ...
            try  {
                ...
                for (var i = 0; i < IncomingAttacks.length; i++) {
                    try  {
                        ...
                        try  {
                            var IncomingScout = Math.round((Math.round((8 / (diffSec / 60) - 1) * 100) * 10) / 10);
                            var IncomingCav = Math.round((Math.round((10 / (diffSec / 60) - 1) * 100) * 10) / 10);
                            var IncomingInf = Math.round((Math.round((20 / (diffSec / 60) - 1) * 100) * 10) / 10);
                            var IncomingSiege = Math.round((Math.round((30 / (diffSec / 60) - 1) * 100) * 10) / 10);
                            var IncomingBaron = Math.round((Math.round((40 / (diffSec / 60) - 1) * 100) * 10) / 10);
                        ...
                        rowData.push([
                            ...
                            cont,
                            ava.CoordUtils.convertIdToCoodrinates(item.tc),
                            formatIncomingDate(thisObj.serverTime.getStepTime(item.es)),
                            item.pn,
                            item.an,
                            item.cn,
                            ava.CoordUtils.convertIdToCoodrinates(item.c),
                            formatIncomingDate(thisObj.serverTime.getStepTime(item.ds)),
                            IncomingBaron,
                            IncomingSiege,
                            IncomingInf,
                            IncomingCav,
    ...
    members: {
        ...
        toggleCityControls: function () {
            if (citySort.getValue()) {
                qx.core.Init.getApplication().cityBar.citiesSelect.getSortedPlayerCities = (function () {
...
var dung2 = (function () {
    function dung2(_type, level, progress, coords, distance) {
        this._type = _type;
        this.level = level;
...
function RaidingWindowEnumProperties(rw, func) {
    for (var i in rw) {
        if (rw.hasOwnProperty(i)) {
            ...
            if ((typeof rw[i] != "function" && typeof rw[i] != "object") && i[0] != '$' && i[0] != '_') {
                func(rw, i);
            }
        }
    }
        ...
        fillLoginTable: function (isOk, result) {
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                rowData.push([
                    ...
                    item.p,
                    item.c,
                    (roles != null ? roles[item.r].Name : item.r),
                    dateFormat.format(loginDate),
                    (this.donations.hasOwnProperty(item.n) ? this.donations[item.n][0] : ""),
                    (this.donations.hasOwnProperty(item.n) ? this.donations[item.n][1] : ""),
                    item.ra
                ]);
            }
            if (rowData.length == 0) {
...
qx.Class.define("ava.RaidingWindow", {
    ...
    members: {
        ...
        createDungeonPage: function () {
            ...
            sel.add(new qx.ui.form.ListItem("really null"));
                ...
                if (objectData) {
                    for (var obj in objectData) {
                        ...
                        if (o.Type == 2) {
                            ...
                            var max = rw.dungeonProgressData[dpt][dpl][dpp][0];
...
function CreateTree(rw, win) {
    ...
    tree.nodeSetLabelStyle(te, "background-color: red; " + "color: white;" + "font-weight: bold;");
    ...
    */
    function (e) {
        ...
        selectedNodes.forEach(function (c) {
            return dataModel.setColumnData(c.nodeId, 2, "sel ");
        });
    });
}
